                    ...
                    <div class="modal-body">
                        <div class="row">
                            @foreach ($questions as $question)
                                <div class="col-md-12">
                                    <h4><strong>Mashq
                                            turi:</strong> {{ $question['homework']['homeworkTypes'][0]['name'] ?? null }}
                                    </h4>
                                    ...
                                    <div class="form-group position-relative">
                                        <label class="w-100">{{ $task_name }}
                                            <span class="position-absolute"
                                                  style="right: 10px; color: #888;">{{ $task_value }}</span>
                                        </label>
                                        <input type="text" name="answers[{{ $task_name }}]" class="form-control"
                                               value="{{ old('answers.' . $task_name, is_array($model['answers'] ?? null)
                                                ? ($model['answers'][$task_name] ?? '')
                                                : (json_decode($model['answers'] ?? '{}', true)[$task_name] ?? '')
                                                ) }}" required>
                            ...
                            <input type="hidden" name="total_questions"
                                   value="{{ $questions->sum(fn($q) => count($q['questions'])) }}">
                            <input type="hidden" name="homework_id" value="{{ $question['homework']['id'] }}">
